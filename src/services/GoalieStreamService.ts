...
export interface GoalieOutlook {
  ...
  gaa: number | null; save_pct: number | null; wins: number; stream_score: number; reason: string;
}

const round = (n: number, p = 1) => Math.round(n * 10 ** p) / 10 ** p;
const surname = (name: string) => name.trim().split(/\s+/).pop() ?? name;

/** Stream score, 0–100: expected starts against four (70%) plus how soft the opposition is (30%). */
export function streamScore(expectedStarts: number, avgAttack: number | null): number {
  return Math.round((Math.min(expectedStarts, 4) / 4) * 70 + (100 - (avgAttack ?? 50)) * 0.3);
}
...
export async function goalieStreams(opts: { look_ahead_days?: number; today?: string; roster?: StoredPlayer[]; top_n?: number } = {}) {
  ...
  const outlook = (g: { player_id: string; name: string; team: string; stats?: any }): GoalieOutlook => {
    ...
    return {
      ...
      stream_score: streamScore(expected, avg),
  ...
  return {
    ...
    candidates,
    take: best
      ? `${best.name} (${best.club}): ${best.games} games, ${best.soft_nights} against weak attacks, and he started ${Math.round(best.start_share * 100)}% of his team's games last season. Best stream on the board.`
      : 'No goalie has games in this window.',
    method: {
      stream_score: 'expected starts against four × 70, plus (100 − average opponent attack) × 0.3; expected starts = games in window × last season\'s start share',
